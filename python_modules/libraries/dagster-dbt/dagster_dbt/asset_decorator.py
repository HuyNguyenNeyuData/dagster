...
def get_dbt_multi_asset_args(
    ...
    for unique_id, parent_unique_ids in dbt_unique_id_deps.items():
        ...
        with disable_dagster_warnings():
            ...
            partition_mapping = dagster_dbt_translator.get_partition_mapping(dbt_resource_props)
            if partition_mapping and has_self_dependency(dbt_resource_props):
                experimental_warning("+meta.dagster.has_self_dependency")

                deps.add(AssetDep(asset=asset_key, partition_mapping=partition_mapping))
