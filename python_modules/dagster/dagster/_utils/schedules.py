...
def _replace_hour_and_minute(pendulum_date: pendulum.DateTime, hour: int, minute: int):
    try:
        new_time = pendulum.datetime(
            pendulum_date.year,
            pendulum_date.month,
            pendulum_date.day,
            hour,
...
def _find_previous_daily_schedule_time_matching_cron_string(
    minute: int, hour: int, pendulum_date: pendulum.DateTime
) -> pendulum.DateTime:
